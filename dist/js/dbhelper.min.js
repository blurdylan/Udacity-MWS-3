...
static fetchReviewsById(restaurant_id){return new Promise((resolve,reject)=>{console.log(`${this.REVIEWS_URL}?restaurant_id=${restaurant_id}`);this._fetchObject(`${this.REVIEWS_URL}?restaurant_id=${restaurant_id}`).then(reviews=>{resolve(reviews);}).catch(error=>{reject(error);});});}
...
static postReview(id,name,rating,comments){let reviewCreated={restaurant_id:id,name:name,rating:rating,comments:comments};axios.post(this.REVIEWS_URL,reviewCreated).then(function(response){DBHelper.addReviewToDb(reviewCreated);console.log(`${name} your review was successfully posted`,response);}).catch(function(error){if(error){DBHelper.addReviewToDb(reviewCreated);console.log("review added to database. Will be posted when online");}
console.log(error);});}
...
static thumbSrcForRestaurant(restaurant){if(restaurant.photograph){return`./img/${restaurant.photograph}-thumb.jpg`;}else{return"./img/restaurant-placeholder-thumb.jpg";}}
...
static addReviewToDb(review){this.dbPromise.then(db=>{const tx=db.transaction(this.DB_REVIEW_STORE_NAME,"readwrite");const store=tx.objectStore(this.DB_REVIEW_STORE_NAME);store.put(review,review.name);return tx.complete;});}
static removeReviewFromDb(review){this.dbPromise.then(db=>{const tx=db.transaction(this.DB_REVIEW_STORE_NAME,"readwrite");const store=tx.objectStore(this.DB_REVIEW_STORE_NAME);console.log(store);store.delete(review.name);return tx.complete;});}

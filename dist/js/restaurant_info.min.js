...
"use strict";let restaurant;let map;let reviews;function updateOnlineStatus(){console.log("User is online");if(navigator.onLine){DBHelper.getReviewsFromDb().then(reviews=>{if(reviews){reviews.forEach(review=>{DBHelper.postReview(review.restaurant_id,review.name,review.rating,review.comments);});var resto=self.restaurant;console.log(resto.id);DBHelper.fetchReviewsById(resto.id).then(reviews=>{self.reviews=reviews;resetReviews(reviews);fillReviewsHTML();}).catch(error=>{alert(error);});}});}}
function updateOfflineStatus(){console.log("User is offline");}
window.addEventListener("online",updateOnlineStatus);window.addEventListener("offline",updateOfflineStatus);window.onload=function(){};window.initMap=()=>{fetchRestaurantFromURL().then(restaurant=>{self.restaurant=restaurant;self.map=new google.maps.Map(document.getElementById("map"),{zoom:16,center:restaurant.latlng,scrollwheel:false});fillRestaurantHTML();DBHelper.mapMarkerForRestaurant(self.restaurant,self.map);fetchReviewsFromURL().then(reviews=>{self.reviews=reviews;fillReviewsHTML();}).catch(error=>{alert(error);});}).catch(error=>{alert(error);});};const fetchRestaurantFromURL=callback=>{return new Promise((resolve,reject)=>{if(self.restaurant){resolve(self.restaurant);return;}
